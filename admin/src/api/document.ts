...
import type { DocumentRootRequest, DocumentRequest, DocumentTreeVO } from "@/types/document";
...
export const getDocumentTreeByIDAPI = (id: string):Promise<Response<DocumentTreeVO[]>> => {
  return request.get(`/document/tree?document_id=${id}`);
};


export const getAllRootDocAPI = ():Promise<Response<DocumentTreeVO[]>> => {
